...
<html>
    ...
    <div style="margin:100px 100px 10px;">
        ...
        if ($_POST) {
            $_SESSION["tripID"] = $_POST['ReserveButton'];
//$query = mysqli_query($connection, "SELECT * FROM Trip WHERE TripID=2");
            $query = $connection->query("SELECT * FROM Trip WHERE TripID=" . $_SESSION['tripID']);
            if ($query) {
                while ($row = mysqli_fetch_array($query)) {
                    $capacity = $row['Capacity'];
                    echo '<form action="ReserveTicketContinue.php" method="POST">';
                    for ($i = 1; $i <= $capacity; $i++) {
                        //$query2 = mysqli_query($connection, "SELECT * FROM Reservation WHERE TripID=2 && SeatID=".$i);
                        $query2 = $connection->query("SELECT * FROM reservation WHERE TripID=" . $_SESSION['tripID'] . " && SeatID=$i");
                        //$query3 = mysqli_query($connection, "SELECT * FROM Ticket WHERE TripID=2 && SeatNumber=".$i." && Cancelled=0");
		...
		<br><br><input type="submit" value="Reserve"><br></form>';
                }
            }
        } else {
