...
if($user){
	...
	while($row2 =mysqli_fetch_array($user)){
		...
		$query= mysqli_query($connection, "SELECT * FROM message WHERE FromID=".$userID);
		if($query){
			while($row= mysqli_fetch_array($query)){
				echo '<div style="background-color:white;color:black;margin:10px;padding:10px;border-radius:5px;font-size:20px;border:2px solid black;">From:Officer <br> Message: '.$row['Content'].'<br> To: '.$row['ToID'].' <br></div>';
			}
		}
	}
}
